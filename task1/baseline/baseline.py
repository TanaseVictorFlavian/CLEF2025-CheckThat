...
def sanitize_and_check_filepath(filepath: AnyStr) -> Path:
...
def run_sbert_lr_baseline(
    data_dir: Path, train_filepath: Path, test_filepath: Path
) -> Path:
    ...
    model.fit(
        X=vect.encode(train_data["sentence"].values), y=train_data["label"].values
    )
...
if __name__ == "__main__":
    ...
    logging.info(f"""Running baseline with following configuration: 
                 ...
                 Test: {test_filepath}""")
    run_sbert_lr_baseline(
        data_dir=data_dir, test_filepath=test_filepath, train_filepath=train_filepath
    )
