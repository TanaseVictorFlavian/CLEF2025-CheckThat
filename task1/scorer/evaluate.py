...
def validate_files(pred_file: AnyStr, gold_file: AnyStr):
    ...
    if not (
        "label" in pred_data
        and "sentence_id" in pred_data
        and "sentence_id" in gold_data
    ):
    ...
    if not (
        (len(pred_values) == 2 and "SUBJ" in pred_values and "OBJ" in pred_values)
        or (len(pred_values) == 1 and ("OBJ" in pred_values or "SUBJ" in pred_values))
    ):
    ...
    if "label" not in gold_data:
        logging.error(
            "Error! No labels in the gold data file. Impossible to proceed with evaluation"
        )
        return False
...
def evaluate(whole_data):
    ...
    m_prec, m_rec, m_f1, m_s = precision_recall_fscore_support(
        gold_values, pred_values, average="macro", zero_division=0
    )
    p_prec, p_rec, p_f1, p_s = precision_recall_fscore_support(
        gold_values, pred_values, labels=["SUBJ"], zero_division=0
    )
